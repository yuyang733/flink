...
public class RecoverableMultipartUploadImpl implements RecoverableMultipartUpload {
	...
	public void uploadPart(RefCountedFSOutputStream file) throws IOException {
		...
		uploadThreadPool.execute(new UploadTask(this.cosAccessHelper, currentUploadInfo, file, future));
	}
	...
	public COSRecoverable snapshotAndGetRecoverable(
		@Nullable RefCountedFSOutputStream incompletePartFile)
		throws IOException {
		...
		final List<PartETag> completedParts = this.currentUploadInfo.getCopyOfEtagsOfCompleteParts();
		...
		if (null == incompletePartObjectName) {
			...
			return new COSRecoverable(uploadId, objectName, completedParts, sizeInBytes, incompletePartObjectName, incompletePartFile.getPos());
		}
	}

	@Nullable
	private String safelyUploadSmallPart(
		@Nullable RefCountedFSOutputStream fsOutputStream) throws IOException {
		...
		try {
			this.cosAccessHelper.putObject(incompletePartObjectName, fsOutputStream.getInputFile(),null);
		} finally {
	...
	private PartETag awaitPendingPartUploadToComplete(
		CompletableFuture<PartETag> upload) throws IOException {
